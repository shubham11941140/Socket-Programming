...
def encrypt(plaintext_bytes):
    ...
    alicePubKey = load_pem_public_key(open(CURR_DIR + '/client-rsapub.pem', 'rb').read(),default_backend())
...
def decrypt(ciphertext_bytes):
    CURR_DIR = dirname(realpath(__file__))
    alicePrivKey = load_pem_private_key(open(CURR_DIR + '/server-rsapvt.pem', 'rb').read(), server_password, default_backend())
